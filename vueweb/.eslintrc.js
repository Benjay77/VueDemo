...
module.exports = {
  ...
  extends: [
    ...
    "plugin:vue/recommended", // 额外添加的规则可查看 https://vuejs.github.io/eslint-plugin-vue/rules/
    /**
     * 「airbnb，爱彼迎」
  ...
  rules: {
    ...
    "dot-location": [2, "property"], // 强制在点号之前换行 成员表达式中的点应与属性部分位于同一行
    "dot-notation": 2, // 点号和字面量，优先使用点号
    //eqeqeq: [2, "smart"], //  要求使用 === 和 !==
    eqeqeq: [2, "always", { null: "ignore" }], // 强制使用全等
    ...
    "no-undef": 2, // 禁用未声明的变量
    "no-unused-vars": 2, //不能有声明后未被使用的变量或参数{vars: "all",args: "none"}
    ...
    "func-style": [2, "declaration", { allowArrowFunctions: true }], //强制 function 声明或表达式的一致性:需要使用函数表达式而不是函数声明
    "function-paren-newline": [2, "consistent"], // 强制在函数括号内使用一致的换行
    ...
    "multiline-comment-style": 0, //[2, "starred-block"], // 强制对多行注释使用特定风格
    ...
    "vue/singleline-html-element-content-newline": 2, // 单行html元素内容在新的一行
    "vue/multiline-html-element-content-newline": 2, // 多行html元素内容在新的一行
    "vue/html-closing-bracket-newline": 0, // html右括号在新的一行
    "vue/no-v-html": 2, // 不使用v-html
    ...
    "vue/name-property-casing": [2, "PascalCase"], //JS/JSX 中的组件名始终是 PascalCase
    "vue/prop-name-casing": [2, "camelCase"], //prop名大小写
    "vue/require-default-prop": 0, // 不检查默认属性
    "vue/require-prop-types": 0, // 不检查默认类型
    "vue/require-v-for-key": 2, //v-for设置键值
